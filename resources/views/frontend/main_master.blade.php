...
<script>
   ...
   {
      $price = $('.price-slider').val(); //get the value of slider
      $ok= $price.split(','); //split the slider value into array
      console.log($ok);

      $selling_price = $('[class="Sprice"]'); //all of all product price span
      $product = $('[id="forhide"]'); //array of all product div
      
      $ma  = $('#forhide');
      
      
      console.log($product);
      $.each($selling_price, function( index, value ) {
           

            if((Number($ok[0]) <=Number($selling_price[index].innerText))&&(Number($selling_price[index].innerText) <= Number($ok[1])))
            {
